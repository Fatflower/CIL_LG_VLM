...
class Finetune_IL(BaseLearner):
        ...
        for _, inputs, targets in test_loader:
            ...
            if ret_pred_target:
                if self._memory_bank != None and self._apply_nme:
                    nme_pred, nme_max_scores = self._memory_bank.KNN_classify(task_begin, task_end, feature_outputs['features'])
    ...
    def eval_task(self):        
        ...
        if self._is_openset_test and self._cur_task < self._nb_tasks-1:
            ...
            cnn_openset_score = cnn_pred_score.copy() if not (cnn_pred_score is None) else None
            nme_openset_score = nme_pred_score.copy() if not (nme_pred_score is None or len(nme_pred_score) == 0) else None

            y_true = np.delete(y_true, openset_idx)
            cnn_pred = np.delete(cnn_pred, openset_idx)
            cnn_pred_score = np.delete(cnn_pred_score, openset_idx) if not (cnn_pred_score is None) else None
            nme_pred = np.delete(nme_pred, openset_idx) if not(nme_pred is None or len(nme_pred) == 0) else None
            nme_pred_score = np.delete(nme_pred_score, openset_idx) if not (nme_pred_score is None or len(nme_pred_score) == 0) else None
    ...
    def get_til_pred_target(self, model, test_dataset):
        ...
        for task_id in range(self._cur_task + 1):
            ...
            if not (nme_pred is None or len(nme_pred) == 0):
                nme_pred_result.append(nme_pred)            
                nme_predict_score.append(nme_pred_score)
        ...
        if len(nme_pred_result) > 0:
            nme_pred_result = np.concatenate(nme_pred_result)
            nme_predict_score = np.concatenate(nme_predict_score)
    ...
    def _get_optimizer(self, params, config, is_init:bool):
        ...
        if is_init:
            if config.opt_type == 'sgd':
                ...
                self._logger.info('Applying sgd: lr={}, momenton={}, weight_decay={}, nesterov={}'.format(self._init_lrate, self._init_opt_mom, self._init_weight_decay, self._init_nesterov))
            ...
            if config.opt_type == 'sgd':
                ...
                self._logger.info('Applying sgd: lr={}, momenton={}, weight_decay={}, nesterov={}'.format(config.lrate, config.opt_mom, config.weight_decay, config.nesterov))
            elif config.opt_type == 'adam':
                ...
                optimizer = optim.AdamW(params, lr=config.lrate,
                                       weight_decay=0 if config.weight_decay is None else config.weight_decay)
                self._logger.info('Applying adamw: lr={}, weight_decay={}'.format(config.lrate, config.weight_decay))
            else: 
                raise ValueError('No optimazer: {}'.format(config.opt_type))
        return optimizer
    ...
    def _get_scheduler(self, optimizer, config, is_init:bool):
        ...
        if is_init:
            ...
            # elif config.scheduler == 'adamw_cos':
            #     scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=config.epochs*config.iters_per_epoch)
            #     self._logger.info('Applying cos scheduler: T_max={}'.format(config.epochs*config.iters_per_epoch))
