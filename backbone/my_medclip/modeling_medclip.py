...
class MedCLIPVisionModelViT(nn.Module):
    ...
    def __init__(self, checkpoint=None, medclip_checkpoint=None) -> None:
        ...
        model = AutoModel.from_pretrained(self.vit_type)
        self.model = LoraModel(model, config, "default")
...
# class my_MedCLIPProcessor(CLIPProcessor):
#     '''
#     A processor that takes texts and provides inputs for
#     `MedCLIPModel`.
#     '''
#     tokenizer_class = ("BertTokenizer", "BertTokenizerFast")
#     def __init__(self):
...
#         tokenizer = AutoTokenizer.from_pretrained(constants.BERT_TYPE)
#         tokenizer.model_max_length = 77
#         super().__init__(image_processor=None,tokenizer=tokenizer)




class clip_ood_cil_med_clip(nn.Module):
    def __init__(self, logger,  pretrained_weights_path):
        super().__init__()
        self._logger = logger

        model = MedCLIPModel(vision_cls=MedCLIPVisionModelViT)
        model.from_pretrained(pretrained_weights_path)
        model = model.cuda()
